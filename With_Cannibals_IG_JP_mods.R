...
fw.matrices <- readRDS("C:/Users/Justin/Documents/Data/FW Survey Data/FW survey analysis/WebBuilder predicted FW matrices.rds")
...
m <- t(fw.matrices$Italian)
    ...
    b <- matrix(c(rep(1,5), rep(0, nrow(m)-5)), nrow = 1, ncol = nrow(m))
...
nbiter = 100  # number of iteration for random parameterization of interaction strengths (the higher the more accurate the final ranking of interactions)
...
for(i in 1:nbiter)
{
  ...
  J = jacobian_with_cannibals(m=m) 
  
  #... Calculate the real part of the dominant eigenvalue (NB: the first line remove the warning saying we are losing the imaginary part information)
  options(warn=-1)
  stab[i] = max(as.double(eigen(J)$values))
  options(warn=0)
})
...
for (j in 1:length(links)){
  ...
  m2 [links_ij[j,1],links_ij[j,2]] = 0
	...
	for(i in 1:nbiter)
	{
	  ...
	  J2 <- jacobian_with_cannibals(m=m2) # Parameterize the interaction strengths
	  options(warn=-1)
	  stab_changed[i] <- max(as.double(eigen(J2)$values)) # Calculate the real part of the dominant eigenvalue
	  options(warn=0)
	}
...
pdf("italia_test_cannibals_seed13000_nbiter100.pdf")
